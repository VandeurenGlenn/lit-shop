...
export class ImagesDialog extends LitElement {
  ...
  get #dialog() {
    return this.renderRoot.querySelector('custom-dialog');
  }
    ...
    const onchange = async (event) => {
      ...
      const files = await Promise.all(
        ...
        `;
          item.onclick = () => {
            this.shadowRoot
              .querySelector('section[route="file"]')
              .removeChild(item);
          };
          this.renderRoot
            .querySelector('section[route="file"]')
    ...
    return html`
      ...
      <form id="content-form" slot="content">
        <strong
          ...
          <span class="deletehash" deletehash=${deletehash}>${deletehash}</span
          >?</strong
        >
      </form>

      <flex-row slot="actions">
        <md-text-button form="content-form" value="cancel"
          >cancel</md-text-button
        >
        <flex-one></flex-one>
        <md-text-button form="content-form" value="submit"
  ...
  #createAlbumDialogTemplate() {
    return html`
      ...
      <flex-row slot="actions">
        <custom-button action="cancel" label="cancel"></custom-button>
        <flex-one></flex-one>
        <custom-button action="submit" label="submit"></custom-button>
      </flex-row>
    `;
  }
    ...
    new Promise((resolve, reject) => {
      const action = ({ detail }) => {
        ...
        const inputFields = Array.from(
          this.renderRoot.querySelectorAll('[input-field]')
        ...
        if (!this.#image.type) {
          const inputField = this.renderRoot.querySelector(
            '[input-field]'
          ) as HTMLInputElement;
        ...
        const image = {
          ...
          data: Array.isArray(this.#image.data)
            ? [...this.#image.data]
            : this.#image.data,
        };
